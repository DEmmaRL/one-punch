...
FROM
  (
    ...
    LEFT JOIN [Reglas T - MEC] AS [Reglas T - MEC_1] ON Tbl_cTMEC_00.MPFra = [Reglas T - MEC_1].ExceptoF
  )
  LEFT JOIN Temp_Vigencia ON [Reglas T - MEC].Vigencia = Temp_Vigencia.Vigencia
WHERE
  (
    (
      ([Reglas T - MEC].[T-MEC])= 'CSCPE'
    )
    AND (
      ([Reglas T - MEC_1].ExceptoF)= '85030099'
      Or ([Reglas T - MEC_1].ExceptoF)= '85389001'
      Or ([Reglas T - MEC_1].ExceptoF)= '85389001'
    )
    AND (
      (
